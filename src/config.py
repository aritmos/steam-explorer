...
class Config:
    def __init__(self):
        try:
            # set the config dir to the same folder as this script
            # (i.e. the scripts directory)
    ...
    def __getattr__(self, name) -> str:
        ...
        if name == "db_uri":
            ...
            if host == "$winhost":
                command = "rg nameserver /etc/resolv.conf | awk '{print $2}'"
                host = subprocess.run(command, shell=True,
